...
class QSS2D(nn.Module):
    def __init__(
        ...
        self.x_base_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_base_proj], dim=0))  # (K=4, N, inner)
        del self.x_base_proj
        ...
        self.x_oth_proj_weight = nn.Parameter(torch.stack([t.weight for t in self.x_oth_proj], dim=0))  # (K=4, N, d_state)
        del self.x_oth_proj
    ...
    def forward_core(self, x: torch.Tensor):
        B, T, C, H, W = x.shape
        # B, C, H, W = x.shape    # bt, c, h, w
        L = H * W
        K = 4

        x_base = x[:, 0]    # b, c, h, w
        x_oth = x[:, 1:].mean(1)    # b, c, h, w

        # (x_base -> dts, Bs) & initialize As, Ds
        x_base_hwwh = torch.stack([x_base.view(B, -1, L), torch.transpose(x_base, dim0=2, dim1=3).contiguous().view(B, -1, L)], dim=1).view(B, 2, -1, L)
        xs_base = torch.cat([x_base_hwwh, torch.flip(x_base_hwwh, dims=[-1])], dim=1) # (B, 4, 360, L)
        x_base_db = torch.einsum("b k d l, k c d -> b k c l", xs_base.view(B, K, -1, L), self.x_base_proj_weight)
        dts, Bs = torch.split(x_base_db, [self.dt_rank, self.d_state], dim=2)
        dts = torch.einsum("b k r l, k d r -> b k d l", dts.view(B, K, -1, L), self.dt_projs_weight)
        xs_base = xs_base.float().view(B, -1, L)
        ...
        # x_oth -> Cs
        x_oth_hwwh = torch.stack([x_oth.view(B, -1, L), torch.transpose(x_oth, dim0=2, dim1=3).contiguous().view(B, -1, L)], dim=1).view(B, 2, -1, L)
        xs_oth = torch.cat([x_oth_hwwh, torch.flip(x_oth_hwwh, dims=[-1])], dim=1) # (B, 4, 360, L)
        x_oth_c = torch.einsum("b k d l, k c d -> b k c l", xs_oth.view(B, K, -1, L), self.x_oth_proj_weight)
    ...
    def forward(self, x: torch.Tensor, **kwargs):
        ...
        xz = self.in_proj(x.view(b*t, c, h, w).permute(0, 2, 3, 1))
        x, z = xz.chunk(2, dim=-1)

        # x: [bt, h, w, c]
        x = x.permute(0, 3, 1, 2).contiguous()  # bt, c, h, w
        x = self.act(self.conv2d(x))
        y1, y2, y3, y4 = self.forward_core(x.view(b, t, -1, h, w))
...
class VSSBlock(nn.Module):
    def __init__(
        ...
        self.self_attention = QSS2D(d_model=hidden_dim, d_state=d_state,expand=expand,dropout=attn_drop_rate, **kwargs)
    ...
    def forward(self, input, x_size):
        ...
        input = input.view(b*t, h, w, c).contiguous()   # [BT, H, W, C]
        x = self.ln_1(input).permute(0, 3, 1, 2).contiguous().view(b, t, c, h, w)  # [B, T, C, H, W]
...
class QMambaIR(nn.Module):
    ...
    def forward_features(self, x):
        ...
        x = self.patch_embed(x.view(b*t, c, h, w)) # N,L,C

        x = self.pos_drop(x).permute(0, 2, 1).view(b*t, -1, h, w).view(b, t, -1, h, w)  # b, t, c, h, w
    ...
    def forward(self, x):
        ...
        if self.upsampler == 'pixelshuffle':
            ...
            x = self.align(x.view(b, t, -1, *x.shape[-2:])) # [b, t, c, h, w]
