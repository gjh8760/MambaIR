# Code Implementation of the BombaBSR Model
import math
import torch
...
class BOSS2D(SS2D):
    ...
    def forward_core(self, x: torch.Tensor):
        T = self.burst_num
        x = x.view(-1, T, *x.shape[1:]) # [B, T, C, H, W]
        B, T, C, H, W = x.shape
        L = H * W
        K = 4
        
        x_hwt = x.permute(0, 2, 3, 4, 1).contiguous()   # [B, C, H, W, T]
        x_wht = x_hwt.transpose(2, 3).contiguous() # [B, C, W, H, T]
        x_hwt_wht = torch.stack([x_hwt.view(B, C, L*T), x_wht.view(B, C, L*T)], dim=1).view(B, 2, C, L*T)   # [B, 2, C, HWT]
        xs = torch.cat([x_hwt_wht, torch.flip(x_hwt_wht, dims=[-1])], dim=1)    # [B, 4, C, HWT]

        x_dbl = torch.einsum("b k d l, k c d -> b k c l", xs.view(B, K, -1, L*T), self.x_proj_weight)
        dts, Bs, Cs = torch.split(x_dbl, [self.dt_rank, self.d_state, self.d_state], dim=2)
        dts = torch.einsum("b k r l, k d r -> b k d l", dts.view(B, K, -1, L*T), self.dt_projs_weight)
        xs = xs.float().view(B, -1, L*T)
        dts = dts.contiguous().float().view(B, -1, L*T) # (b, k * d, l)
        Bs = Bs.float().view(B, K, -1, L*T)
        Cs = Cs.float().view(B, K, -1, L*T) # (b, k, d_state, l)
        Ds = self.Ds.float().view(-1)
        As = -torch.exp(self.A_logs.float()).view(-1, self.d_state)
        dt_projs_bias = self.dt_projs_bias.float().view(-1) # (k * d)
        out_y = self.selective_scan(
